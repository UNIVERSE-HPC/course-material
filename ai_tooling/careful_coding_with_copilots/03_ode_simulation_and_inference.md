...
```
The repressilator paper can be found in the `docs/` folder. You should read box 1 on page 3, which talks about the ordinary differential equation model. As you read, note the form of the differential equations for mRNA and protein and which parameters are defined (α, α₀, β, n). If you have not encoutered it before, you may be interested in the [Hill equation](https://en.wikipedia.org/wiki/Hill_equation_(biochemistry)) which is used in the ODE model to describe binding kinetics. 
